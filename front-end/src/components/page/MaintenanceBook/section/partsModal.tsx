...
const MtPartsModal: NextPage<_pPartsSetProps> = (props) => {
  ...
  const addPartToSet = () => {
    ...
    let newList: MainWork[] = props.workList;
    let inputSumList: number[] = props.inputSum;
    for (let i = 0; i < selectPart.length; i++) {
      for (let j = 0; j < props.workList?.length; j++) {
        if (selectPart[i].code === props.workList[j]?.code) {
          ...
          inputSumList = props.inputSum.map((num, index) =>
            index === j
              ? props.workList[j].price * (props.workList[j].quantity + 1)
              : num
          );
        } else {
          console.log("bye");
    ...
    console.log(newList);
    props.setInputSum(inputSumList);
    props.setWorkList(newList);
